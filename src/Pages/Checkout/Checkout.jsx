...
import {
  MapPin, Plus, ChevronRight, CreditCard, Smartphone, Banknote,
  Lock, Truck, ShieldCheck, Navigation, CheckCircle2, Package,
  ArrowLeft, Loader2, ArrowRight, Info, Gift, Sparkles, TrendingUp,
} from "lucide-react";
...
function calcPricing(subtotal, paymentMethod, isFirstOrder = false) {
  ...
  if (isFirstOrder) {
    deliveryCharge = 0; freeDelivery = true; deliveryNote = "first_order";
  } else if (subtotal >= FREE_DELIVERY_THRESHOLD) {
    deliveryCharge = 0; freeDelivery = true; deliveryNote = "threshold_met";
  } else {
    deliveryCharge = DELIVERY_CHARGE; freeDelivery = false; deliveryNote = "charged";
  }
  const amountForFreeDelivery = (!isFirstOrder && !freeDelivery)
    ? Math.ceil(FREE_DELIVERY_THRESHOLD - subtotal) : null;
  ...
  let nudge = null;
  if (isFirstOrder) {
    nudge = { type: "first_order", text: `🎁 Free delivery on your first order — you save ₹${DELIVERY_CHARGE}!` };
  } else if (amountForFreeDelivery) {
    nudge = { type: "upsell", text: `Upgrade to ₹449 pack and get FREE delivery! Save ₹${DELIVERY_CHARGE}` };
  }
  return {
    subtotal, baseAmount, gstAmount, deliveryCharge, deliveryNote,
    codCharge, totalAmount, freeDelivery, isFirstOrder,
    deliverySaved, amountForFreeDelivery, nudge,
  };
}

const EMPTY_PRICING = {
  subtotal: 0, baseAmount: 0, gstAmount: 0, deliveryCharge: 0,
  deliveryNote: "charged", codCharge: 0, totalAmount: 0,
  freeDelivery: false, isFirstOrder: false, deliverySaved: 0,
  amountForFreeDelivery: null, nudge: null,
};
...
───────────────────────────────────────────── */
function PaymentOption({ value, selected, onChange, icon, label, sublabel, badge }) {
  const Icon = icon;
  return (
    <label
      className={`
    ...
    >
      ...
      />
      {/* Icon */}
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${selected ? "bg-orange-100" : "bg-stone-100"}`}>
        <Icon className={`w-5 h-5 ${selected ? "text-orange-600" : "text-stone-500"}`} strokeWidth={2} />
      </div>
      {/* Text — min-w-0 is critical to prevent flex child overflow */}
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="flex items-center gap-1.5 flex-wrap">
          <p className={`text-sm font-bold leading-snug ${selected ? "text-orange-700" : "text-stone-700"}`}>
            {label}
          </p>
          {badge && (
        ...
        {sublabel && (
          <p className="text-[11px] text-stone-400 mt-0.5 leading-snug break-words">
            {sublabel}
          </p>
        )}
      </div>
      {/* Radio indicator */}
      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ml-1 ${selected ? "border-orange-500" : "border-stone-300"}`}>
...
function DeliveryRow({ pricing }) {
    ...
    return (
      <div className="flex justify-between items-center gap-2 text-sm">
        ...
        <span className="flex items-center gap-1.5 shrink-0">
          <span className="text-stone-300 line-through text-xs">₹{DELIVERY_CHARGE}</span>
  ...
  return (
    <div className="space-y-0.5">
      ...
      <p className="text-[11px] text-stone-400 text-right">Free on ₹{FREE_DELIVERY_THRESHOLD}+ orders</p>
    </div>
  );
}
  ...
  if (nudge.type === "first_order") {
    return (
      <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl bg-orange-50 border border-orange-200 shadow-sm w-full overflow-hidden">
        ...
        <p className="text-xs font-bold text-orange-800 leading-snug break-words min-w-0">{nudge.text}</p>
      </div>
    );
  }
  return (
    <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 shadow-sm w-full overflow-hidden">
      ...
      <p className="text-xs font-bold text-amber-800 leading-snug break-words min-w-0">{nudge.text}</p>
    </div>
  );
}
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-stone-500 font-medium flex-wrap min-w-0">
        {!selectedAddress && (
          <span className="flex items-center gap-1 whitespace-nowrap">
            <MapPin className="w-3 h-3 text-orange-400 shrink-0" /> Select address
          </span>
        )}
        {!selectedAddress && !paymentMethod && <span className="text-stone-300">·</span>}
        {!paymentMethod && (
          <span className="flex items-center gap-1 whitespace-nowrap">
            <CreditCard className="w-3 h-3 text-orange-400 shrink-0" /> Select payment
          </span>
        )}
      </div>
    </div>
...
function CartItemRow({ item }) {
  return (
    <div className="flex items-center gap-2.5 overflow-hidden">
      {/* Thumbnail */}
      <div className="w-10 h-10 rounded-xl border border-stone-100 bg-stone-50
    ...
    <div className="space-y-2.5">
      ...
      {pricing.isFirstOrder && pricing.deliverySaved > 0 && (
        <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-100 gap-2 overflow-hidden">
          ...
          <span className="text-xs font-black text-emerald-700 shrink-0">₹{pricing.deliverySaved}</span>
        </div>
      )}
      ...
      <p className="text-[10px] text-stone-400 text-center">All prices are MRP inclusive of taxes</p>
      {!paymentMethod && (
...
const Checkout = () => {
  ...
  const cartSubtotal = cart.reduce((sum, i) => sum + (i.variant?.price || 0) * i.quantity, 0);
  ...
  useEffect(() => {
    ...
    setPricing(calcPricing(cartSubtotal, form.paymentMethod, isFirstOrder === true));
  ...
  const buildProductsPayload = useCallback(() => {
    ...
    return cart.map((item) => {
      ...
      const variantIndex = item.variantIndex ??
        item.product.variants?.findIndex((v) => v.weight === item.variant?.weight);
  ...
  const detectLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        } catch { toast.error("Unable to fetch address details"); }
    ...
    try {
      ...
      if (form.paymentMethod === "COD") {
        const res = await API.post("/api/orders/postorders", {
          products, addressId: selectedAddress, paymentMethod: "COD",
        });
      ...
      const orderRes = await API.post("/api/orders/postorders", {
        products, addressId: selectedAddress, paymentMethod: "Online",
      });
      if (!orderRes.data?.success) throw new Error("Order creation failed");
      const orderId = orderRes.data?.data?._id || orderRes.data?.order?._id || orderRes.data?.orderId;
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-3">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-sm font-medium text-stone-500
                       hover:text-stone-800 transition-colors min-h-[44px] min-w-[44px] shrink-0"
          >
      ...
      >
        ...
        */}
        <div className="grid lg:grid-cols-[1fr_390px] gap-6 items-start">
          ...
          <div className="space-y-5 min-w-0">
                  ...
                  !showForm && savedAddresses.length > 0 ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs font-bold text-orange-600
                                 hover:text-orange-500 transition-colors min-h-[44px] px-1 whitespace-nowrap"
                    >
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-stone-500 hover:text-stone-700
                                 transition-colors min-h-[44px] px-1 whitespace-nowrap"
                    >
                      ← Use Saved
                    </button>
                  ) : null
                  ...
                  {savedAddresses.map((a) => (
                    ...
                    >
                      <div className={`mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0
                        ...
                        {selectedAddress === a._id && <div className="w-2.5 h-2.5 rounded-full bg-orange-500" />}
                      </div>
                      <div className="flex-1 min-w-0">
                ...
                <div className="space-y-4">
                  <button
                    ...
                    className="inline-flex items-center gap-2 text-xs font-bold text-orange-600
                               hover:text-orange-500 transition-colors disabled:opacity-60 min-h-[44px]"
                  >
                    {detectingLoc
                      ? <><Loader2 className="w-3.5 h-3.5 animate-spin shrink-0" /> Detecting…</>
                      : <><Navigation className="w-3.5 h-3.5 shrink-0" /> Auto-detect my location</>}
                  </button>
            ...
            <div className="bg-white border border-stone-100 rounded-2xl shadow-sm p-4 sm:p-6 overflow-hidden">
              ...
              <div className="space-y-3">
                <PaymentOption
                  value="Online" selected={form.paymentMethod === "Online"} onChange={onChange}
                  icon={Smartphone} label="Pay Online" badge="No extra charge"
                  sublabel="UPI · Debit/Credit Card · Net Banking · Wallets"
                />
                <PaymentOption
                  value="COD" selected={form.paymentMethod === "COD"} onChange={onChange}
              ...
              <div className="mt-4 flex items-start gap-2 px-3 py-2.5 rounded-xl bg-emerald-50 border border-emerald-100 overflow-hidden">
                ...
                <p className="text-xs text-emerald-700 font-medium leading-snug break-words min-w-0">
            ...
            <div className="lg:hidden bg-white border border-stone-100 rounded-2xl shadow-sm overflow-hidden">
              <div className="px-4 sm:px-5 pt-5 pb-4 border-b border-stone-100">
                ...
                <div className="space-y-3">
                  {cart.map((item, i) => <CartItemRow key={i} item={item} />)}
                </div>
              </div>
              ...
              <div className="mx-4 sm:mx-5 mb-5 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-100 overflow-hidden">
                ...
                <p className="text-xs text-amber-700 font-medium truncate">Estimated delivery in 3–6 business days</p>
              </div>
            </div>
          ...
          <div className="hidden lg:block lg:sticky lg:top-[90px] h-fit space-y-3 min-w-0">
            ...
            <div className="bg-white border border-stone-100 rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="px-5 pb-5">
                  ...
                  {loading
                    ...
                    : <>
                        ...
                        <span className="truncate">
                          {form.paymentMethod === "COD" ? "Place Order" : `Pay ${fmt(pricing.totalAmount)} Securely`}
                        </span>
                ...
                <div className="flex items-center justify-center gap-1.5 mt-2">
                  ...
                  <p className="text-[10px] text-stone-400 truncate">256-bit SSL encrypted · Powered by Razorpay</p>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        {pricing.nudge && (
          <div className="flex items-center gap-1.5 mb-2 overflow-hidden">
            ...
            <p className="text-[11px] font-bold text-amber-800 truncate">{pricing.nudge.text}</p>
          </div>
        )}

        <MobileStepGuard selectedAddress={selectedAddress} paymentMethod={form.paymentMethod} />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {pricing.gstAmount > 0 && (
              <p className="text-[10px] text-stone-400 mt-0.5 truncate">Incl. ₹{pricing.gstAmount} GST</p>
            )}
            {pricing.isFirstOrder && pricing.deliverySaved > 0 && (
          ...
          >
            {loading ? (
              <><Loader2 className="w-4 h-4 animate-spin shrink-0" /><span className="truncate">Processing…</span></>
            ) : (
              <>
                <Lock className="w-4 h-4 shrink-0" />
                <span className="truncate">
