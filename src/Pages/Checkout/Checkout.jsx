...
// src/pages/checkout/CheckoutPage.jsx// src/pages/checkout/CheckoutPage.jsx
...
import { startRazorpayPayment } from "./PaymentHandler";
...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async ({ addressId, paymentMethod }) => {
    ...
    try {
      ...
      const cartCheck = await API.get("/api/cart");
      ...
      const { data } = await API.post("/api/orders/postorders", {
        addressId,
        paymentMethod,
      });
      ...
      const verifiedOrderId = await startRazorpayPayment({
        orderId,
        user,
      });
      ...
      navigate(`/payment-success/${verifiedOrderId}`, {
        replace: true,
      });
    } catch (err) {
